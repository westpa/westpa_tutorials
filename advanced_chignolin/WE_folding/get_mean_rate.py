...
if len(sys.argv) == 3 :
   ...
   print("\n\nPLEASE, PROVIDE THE WESTPA RESAMPLING TIME, TAU, AND THE WIDTH "
         "FOR THE WINDOW-AVERAGING  (BOTH IN SECONDS) AS COMMAND LINE ARGUMENTS, "
...
fluxanl = h5py.File('fluxanl.h5', 'r')
...
FileOut.write("#molecular time [ns]\trate [1/s]\n")
...
for i in range(len(flux)) :
   ...
   FileOut.write("{0:.3f} \t\t {0:.8e} \n".format((i+1)/(1e-9/tau), flux_mean/tau))
###########################################################################################
